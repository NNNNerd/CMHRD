    ...
    for iou_type in iou_types:
        with tempfile.NamedTemporaryFile() as f:
            ...
            res = evaluate_predictions_on_coco(
                ...
                ignore_uncertain, use_iod_for_ignore, eval_standard, gt_file, use_ignore_attr
            )
            results.update(res)
    logger.info(results)
...
def evaluate_predictions_on_coco(
    ...
    ignore_uncertain=False, use_iod_for_ignore=False, eval_standard='coco', gt_file=None, use_ignore_attr=True
):
    import json
    ...
    # ###################### add by hui ##########################################
    if len(gt_file['merge']) > 0:
        from MyPackage.tools.pub.split_and_merge_image import COCOMergeResult
        print('merge result from sub image', json_result_file, gt_file)
        _, json_result_file = COCOMergeResult(use_nms=True, nms_th=0.5)(
            gt_file['sub'],
            json_result_file,
            os.path.split(json_result_file)[0]  # dir
        )
        coco_gt = COCO(gt_file['merge'])
    ##############################################################################
    ...
    coco_eval = COCOeval(coco_gt, coco_dt, iou_type, ignore_uncertain, use_iod_for_ignore, use_ignore_attr)
    coco_eval.evaluate()
