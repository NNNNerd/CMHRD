...
class COCOeval:
    ...
    def summarize(self):
        ...
        def _summarizeDets_vtuavs():
            ...
            stats.append(_summarize(1, iouThr=.5, areaRng='small', maxDets=self.params.maxDets[2]))
            stats.append(_summarize(1, areaRng='medium', maxDets=self.params.maxDets[2]))
            stats.append(_summarize(1, iouThr=.5, areaRng='medium', maxDets=self.params.maxDets[2]))
            stats.append(_summarize(1, areaRng='large', maxDets=self.params.maxDets[2]))
            stats.append(_summarize(1, iouThr=.5, areaRng='large', maxDets=self.params.maxDets[2]))
        ...
        if iouType == 'segm' or iouType == 'bbox':
            if self.params.EVAL_STRANDARD == 'tiny' or self.params.EVAL_STRANDARD == 'noaa': 
                summarize = _summarizeDets_tiny
            elif self.params.EVAL_STRANDARD == 'vtuav_small':
                summarize = _summarizeDets_vtuavs
...
class Params:
    ...
    def setDetParams(self):
        ...
        elif eval_standard== 'noaa':
            self.iouThrs = np.linspace(
                .3, 0.75, int(np.round((0.75 - .3) / .05)) + 1, endpoint=True)
            self.recThrs = np.linspace(.0, 1.00, int((1.00 - .0) / .01) + 1, endpoint=True)
            self.maxDets = [200]
            self.areaRng = [[1 ** 2, 1e5 ** 2]]
            # s = 4.11886287119646
            # self.areaRng = np.array(self.areaRng) * (s ** 2)
            self.areaRngLbl = ['all']
            self.useCats = 1     
        elif eval_standard == 'vtuav_small':
            self.imgIds = []
            self.catIds = []
            # np.arange causes trouble.  the data point on arange is slightly larger than the true value
            self.iouThrs = np.linspace(.5, 0.95, int((0.95 - .5) / .05) + 1, endpoint=True)
