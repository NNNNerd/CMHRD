...
class RFLA_FCOSKDHead(AnchorFreeHead):
    ...
    def loss(self,
        ...
        if cls_scores_tea is not None:
            for _ in range(self.s_gap):
                cls_scores_tea.pop(0)
                bbox_preds_tea.pop(0)
                centernesses_tea.pop(0)
                cls_scores.pop(-1)
                bbox_preds.pop(-1)
                centernesses.pop(-1)
            ...
            for cls_score, bbox_pred, centerness in zip(cls_scores, bbox_preds, centernesses):
                resp = torch.cat([cls_score, \
                                bbox_pred,
                                centerness], dim=1)
                resp = resp.permute(0, 2, 3, 1).reshape(-1, self.num_classes+5)
                resps.append(resp)

            resps_tea = []
            for cls_score_tea, bbox_pred_tea, centerness_tea in zip(cls_scores_tea, bbox_preds_tea, centernesses_tea):
                resp_tea = torch.cat([cls_score_tea, \
                                bbox_pred_tea,
                                centerness_tea], dim=1)
                resp_tea = resp_tea.permute(0, 2, 3, 1).reshape(-1, self.num_classes+5)
                resps_tea.append(resp_tea)

            loss_kd = bbox_pred.sum() * 0
            for i in range(len(resps)):
                if self.gt_mask is not None:
                    gt_mask_ = gt_mask.unsqueeze(1)
                    n = gt_mask_.shape[-1] // cls_scores[i].size()[-1]
                    for _ in range(math.floor(math.log2(n))):
                        gt_mask_ = F.max_pool2d(gt_mask_, 2, 2, ceil_mode=True)
                    
                    gt_mask_ = gt_mask_.permute(0, 2, 3, 1).reshape(-1, 1)
                    loss_kd_ = self.loss_kd(resps[i], resps_tea[i]) * gt_mask_
                    loss_kd += loss_kd_.mean()
                else:
                    loss_kd_ = self.loss_kd(resps[i], resps_tea[i])
                    loss_kd += loss_kd_.mean()
