...
class HRNet(BaseModule):
    ...
    def forward_stagex(self, x, stage_idx):
        x_list = []
        stage_cfg = getattr(self, 'stage'+str(stage_idx)+'_cfg')
        transition = getattr(self, 'transition'+str(stage_idx-1))
        stage = getattr(self, 'stage'+str(stage_idx))
        if isinstance(x, list):
            for i in range(stage_cfg['num_branches']):
                if transition[i] is not None:
                    x_list.append(transition[i](x[-1]))
                else:
                    x_list.append(x[i])
        else:
            for i in range(stage_cfg['num_branches']):
