...
model = dict(
    ...
    bbox_head=dict(
        ...
        feat_channels=256,
        centerness=1,
        gt_mask=dict(alpha=0., img_shape=(180,320,3)),
    ...
    test_cfg=dict(
        ...
        max_per_img=100),
    teacher_cfg='cmhrd_configs/atss_r50_fpn_p3_qls_1x_rtdod_v.py',
    teacher_ckpt='work_dirs/atss_r50_fpn_p3_qls/rtdod/visible-hr/epoch_10_best.pth',
...
dataset_type = 'RTDOD'
data_root = '/home/zhangyan/data/RTDOD/'
img_norm_cfg = dict(
    mean_list=([80.87, 98.29, 94.70], [92.83, 92.83, 92.83]),
    std_list=([65.81, 69.91, 67.04], [44.59, 44.59, 44.59]), to_rgb=True)
train_pipeline = [
    dict(type='LoadImagePairFromFile', spectrals=('visible', 'thermal')),
    dict(type='LoadAnnotations', with_bbox=True),
    dict(type='MultiResize', img_scale=[(1280, 720), (320, 180)], keep_ratio=True, main_scale_idx=2),
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        ann_file=data_root + 'RTDOD_train.json',
        img_prefix=data_root + '',
        pipeline=train_pipeline),
    val=dict(
        type=dataset_type,
        ann_file=data_root + 'RTDOD_test.json',
        img_prefix=data_root + '',
...
work_dir = 'work_dirs/atss_r50_fpn_p3_qls_cmhrd/rtdod'
